...
public class ZhanWuYan_Fire : MonoBehaviour
{
    ...
    {
        if (other.tag == "Box" || other.tag == "SmallBox" || other.tag == "SmallMonster" || other.tag == "ZhanWuYanDiLei")
        {
    ...
    private IEnumerator ApplyDamage()
    {
        ...
        while (targets.Count > 0)
        {
            foreach (Continuous_Harm target in new List<Continuous_Harm>(targets))
            {
                if (target != null)
                {
                    target.TakeDamage(damagePerSecond,ZhanWuYan_Wenzi);
                }
                else
                {

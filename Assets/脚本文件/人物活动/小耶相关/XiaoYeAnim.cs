...
public class XiaoYeAnim : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        if (WhetherMove())
        {
            XiaoYeAnimator.SetBool("isMove", true);
        }
        else if (!WhetherMove()) XiaoYeAnimator.SetBool("isMove", false);
        if (!isMirrored)//ÍùÓÒ¿´
        {
    ...
    private void IsChangeAnim()
    {
        ...
        if (angle <= 145.0f)
        {
            XiaoYeAnimator.SetBool("isAngle", true);
        }
        else if (angle > 145f) XiaoYeAnimator.SetBool("isAngle", false);
    }

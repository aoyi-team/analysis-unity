...
public class DiShiTianArrow : MonoBehaviour
{
    public GameObject AttackParticle;//测试在箱子上面能否实现
    public GameObject DiShiTianWenZi;
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.CompareTag("Box") || collision.CompareTag("SmallMonster") || collision.CompareTag("SmallBox"))
        {
            ...
            if (TargetGameobject.tag == "Box")
            {
                ...
                Instantiate(DiShiTianWenZi, MislePosition, Quaternion.identity);
            ...
            else if (TargetGameobject.tag == "SmallBox")
            {
                ...
                Instantiate(DiShiTianWenZi, FeibiaodPosition, Quaternion.identity);
                Vector2 ParticleRandom = Random.insideUnitCircle * ParticleMover;
    ...
    private void HideTheArrow()
    {
        gameObject.GetComponent<BoxCollider2D>().enabled = false;
        Instantiate(Voice, transform.position, Quaternion.identity);

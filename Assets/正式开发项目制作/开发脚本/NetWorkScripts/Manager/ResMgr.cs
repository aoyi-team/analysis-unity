...
public static class ResMgr
{
    ...
    public static Sprite LoadModeIcon(GameModes mode)
    {
        ...
        // Fallback：缺失图标的模式用相近图标代替
        // paiwei_solo（排位单人）→ paiwei 图标
        // shengcun（生存）→ geren 图标
        string fallbackKey = mode == GameModes.paiwei_solo ? "paiwei" : "geren";
        if (_modesIncoCahe.ContainsKey(fallbackKey))
        {
            Debug.Log($"[ResMgr] 模式 {mode} 无专属图标，使用 fallback: {fallbackKey}");
            return _modesIncoCahe[fallbackKey];
        }
        return null;
    }
    // ================== 英雄皮肤海报加载 ==================
